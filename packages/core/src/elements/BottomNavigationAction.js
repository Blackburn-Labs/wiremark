// @ts-check
import { iconGlyph, centeredLabel } from '../draw.js';
 ...
 * glyph on top with its label centered beneath. The keyless quoted literal is
 * the `label`; `icon=` is a keyed icon NAME drawn as the same placeholder glyph
 * Icon uses (a bordered box with a diagonal mark -- FAMILIES.md icon ruling).
 *
...
/** Icon glyph extent (px) drawn above the label. */
const GLYPH = 24;
...
export default {
  ...
  notes:
    ...
    + 'icon= is an icon NAME drawn via the shared iconGlyph placeholder (no icon PropType). '
  ...
  render: (node, box) => {
    // Glyph centered horizontally near the top; label centered in the band below.
    const gx = box.x + box.w / 2 - GLYPH / 2;
    const gy = box.y + PAD_Y;
    let out = iconGlyph(gx, gy, GLYPH);
