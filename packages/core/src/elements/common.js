 ...
 * @property {{ w?: boolean, h?: boolean } | ((node: ResolvedNode) => { w?: boolean, h?: boolean })} [fill]  // ABSOLUTE-AXIS fill: declare that this element fills the parent content WIDTH (`w`) and/or HEIGHT (`h`), regardless of which is the parent's main/cross axis. The engine maps the requested absolute dim onto the parent axis: a CROSS-dim request stretches to crossAvail (exactly like `block`); a MAIN-dim request flexes like `*` (weight 1) -- but only when the child has NO explicit main size token (a px/%/fill/flex token wins). Additive to `block` (an element with neither keeps today's behavior); inert in grids (like flex weights). Static object or per-node predicate, like `block`/`overlay`/`minSize`. (Drawer: vertical fills height, horizontal fills width -- the SAME declaration works at a column root or inside a row.)
